      ...
      <div className='max-w-3xl mx-auto'>
        ...
        <div className='bg-white rounded-2xl shadow overflow-hidden'>
          ...
          <div className='h-40 md:h-56 bg-gradient-to-r from-indigo-200 via-purple-200 to pink-200'>
            {user.cover_photo && <img src={user.cover_photo} className='w-full h-full obejct-cover' />}
          </div>
          {/* user info */}
          ...
          <div className='bg-white rounded-xl shadow p-1 flex max-w-md mx-auto'>
               {['posts' , 'media' , 'likes'].map((tab) => (
                <button onClick={()=> setActiveTab(tab)} key ={tab} className={`flex-1 px-4 py-2 text-sm font-medium
                  rounded-lg transition-colors curosr-pointer ${activeTab === tab ? "bg-indigo-600 text-white" : "text-gray-600 hover:text-gray-900"}`}>
                ...
                <div className='flex flex-wrap mt-6 max-w-6xl'>
                  {
                    posts.filter((post) => post.image_urls.length > 0).map((post)=>(
                      ...
                      {post.image_urls.map((image ,index)=> (
                          ...
                          <img src={image} key={index} className ='w-64 aspect-video obejct-cover' />

...
import {Verified , MapPin ,PenBox ,Calendar} from 'lucide-react'
    ...
    return (
     ...
     <div className='flex flex-col md:flex-row items-start gap-6'>
      <div className='w-32 h-32 border-4 border-white shadow-lg absolute -top-16 rounded-full overflow-hidden'>
        <img src={user?.profile_picture} alt="" className='absolute inset-0 w-full h-full object-cover rounded-full z-10' />
      </div>
        <div className='w-full pt-16 md:pt-0 md:pl-36'>
              <div className='flex flex-col md:flex-row items-start justify-between'>
                  ...
                  {/* edit button if user is on his profile */}
                  {!profileId && <button onClick={() => setShowEdit(true)} className='cursor-pointer flex items-center gap-2 border border-gray-300 hover:bg-gray-50 px-4 py-2 rounded-lg font-medium transition-colors mt-4 md:mt-0'> 
              ...
              <div className='flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500 mt-4'>
                <span className='flex items-center gap-1.5'>
                    ...
                    {user.location ? user.location : 'Ddd location'}
                </span>
                <span className='flex items-center gap-1.5'>

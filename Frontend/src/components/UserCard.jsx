...
const UserCard = ({ user }) => {
    ...
    return (
        <div key={user._id} className='p-4 pt-6 flex flex-col justify-col justify-between w-72 shadow border border-gray-200 rounded-md'>
            ...
            <div className='flex mt-4 gap-2'>
                <button onClick={handleFollow} disabled={currentUser?.following.includes(user._id)} className='w-full py-2 rounded-md flex justify-center
                     ...
                     hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-shite cursor-pointer'>
                    < UserPlus className='w-4 h-4' />{currentUser?.following.includes(user._id) ? 'Following' : 'Follow'}
                </button>
                {/* connection request / Message request */}
                <button onClick={handleConnectionRequest} className='flex items-center justify-center w-16 border text-slate-500 group ronded-md cursor-pointer active:scale-95 transition'>
                    {
